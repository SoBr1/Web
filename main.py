...
def set_questions():
    global connect, cursor
    quest_list = [("На какой высоте спавнятся алмазы","10","26","77"),
                ("Какая версия майкрафта позднее из этих?","1.12.2","1.7.10","1.5.2"),
                ("кто основал Владимир?","Владимир","Юрий Долгорукий","Чингисхан"),
                ("самый непопулярный год для minecraft","2018","2022","2021"),
                ("в каком измерении материалы для шалкер-сундука","end","nether","просто"),
...
def get_next_question(nummer_victor,now_quest):
    global connect, cursor
    open()
    cursor.execute('''SELECT quest_id FROM links WHERE quiz_id==(?) AND quest_id > (?)''',[nummer_victor,now_quest])
